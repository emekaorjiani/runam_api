    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.searches.update", [$search->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('news_title') ? 'is-invalid' : '' }}" type="text" name="news_title" id="news_title" value="{{ old('news_title', $search->news_title) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('click') ? 'is-invalid' : '' }}" type="number" name="click" id="click" value="{{ old('click', $search->click) }}" step="1" required>
                @if($errors->has('click'))
                    <div class="invalid-feedback">
                        {{ $errors->first('click') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('source') ? 'is-invalid' : '' }}" name="source_id" id="source_id">
                    @foreach($sources as $id => $entry)
                        <option value="{{ $id }}" {{ (old('source_id') ? old('source_id') : $search->source->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
